...
import { MCPWebSocketServer } from "./websocket.js";

// Logging is enabled only if LOG_ENABLED environment variable is set to 'true'
const LOG_ENABLED = process.env.LOG_ENABLED === 'true';
const WS_PORT = parseInt(process.env.WS_PORT ?? "27042", 10);
const HOST = process.env.HOST ?? "127.0.0.1"

// Initialize WebSocket server
const wsServer = new MCPWebSocketServer(WS_PORT);
...
async function runServer() {
    ...
    log("MCP Proxy Sidecar running on stdio");
}
